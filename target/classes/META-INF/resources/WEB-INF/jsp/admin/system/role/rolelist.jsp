	...
	<div style="padding: 15px;" class="layui-anim layui-anim-upbit"/>
	 ...
	 <!--  </form> -->
	  <div class="layui-form">
		  <table class="layui-table" lay-filter="equipment" lay-data="{url:'/${applicationScope.adminprefix }/system/role/list/dataTable', page:true, id:'roleList_Id'}">
		    <thead>
		      <tr>
		        <th lay-data="{field:'roleName', width:'40%', sort: true}">角色名称</th>
		        <th lay-data="{field:'identify', width:'40%', sort: true}">角色分类</th>
		        <th lay-data="{field:'roleid', width:'20%', sort: true, toolbar: '#barDemo',fixed:'right'}">操作</th>
		      </tr> 
		    </thead>
		  </table>
	  </div>
		...
		layui.use(['layer', 'table'], function(){
			...
			table.on('tool(equipment)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				if(layEvent === 'edit') {
                    ...
                    confirminfo("确定要删除吗？",ok);
                    function ok(){
                        $.ajax({
                            type:"post",
                            data:{"roleId": roleId,r: Math.random()},
                            url:"/${applicationScope.adminprefix }/system/role/roleDelete",
                            datatype:"json",
                            success:function(data){
                                if(data.success) {
                                    alertinfo(data.msg, true);
                                }else{
                                    alertinfo(data.msg,false);
                                }
                                table.reload('roleList_Id', {
                                    page: {
                                        curr: 1
