		...
		for (MultipartFile files : filess) {
			...
			if (files.isEmpty()) {
				...
				String newFileName = UUID.randomUUID().toString().replace("-", "")
						+ Tools.getFileExtension(files.getOriginalFilename());// 获取图片名称
				// 图片保存路径
				...
				SiteLogo =SiteLogo.substring(SiteLogo.indexOf("upload")-1).replace("\\", "/");
				// 将所有图片名称按照逗号拼接
	...
	public static List<String> extractNumber(String str){
		...
		Collections.sort(list, new Comparator<String>() {
			...
			public int compare(String o1, String o2) {
				if(DataConvert.ToInteger(o1) > DataConvert.ToInteger(o2)){
					return 1;
				}
				if(DataConvert.ToInteger(o1) == DataConvert.ToInteger(o2)){
					return 0;
